...
def analysis():
    ...
    a2.plot(["age", "trestbps", "chol", "thalach"], [A,D,E,I])
...
#input data
    ...
    prediction, acc = predict(input_data)
    print(prediction)

    if prediction == 0:
        print('The Person does not have a Heart disease')
        report.config(text="Report: 0", fg="#8dc63f")
        report1.config(text=f"{name}, you do not have a heart disease")
        report2.config(text="")
    else:
        print('The Person has Heart disease')
        report.config(text="Report: 1", fg="#ed1c24")
        report1.config(text=f"{name}, you have a heart disease")
        report2.config(text=f"Chance: {int(round(acc*100, 2))}%")
...
cp_combobox=Combobox(Detail_entry, values=['0 = typical angina', '1 = atypical angina', '2 = non-aginal pain', '3 = asymtomatic'], font="arial 12", state="r", width=14)
restecg_combobox=Combobox(Detail_entry, values=['0','1','2'], font="arial 12", state="r",width=11)
slope_combobox=Combobox(Detail_entry, values=['0 = unslopping','1 = flat','2 = downslopping'], font="arial 12",state="r",width=12)
...
report_background.place(x=1110,y=340)

report=Label(root, font="arial 22 bold",bg="white", fg="#8dc63f")
report.place(x=1155,y=525)

report1=Message(root, anchor="w", font="arial 10 bold", bg="white", width = 100)
report1.place(x=1150,y=585, width=150)

report2=Label(root, font="arial 10 bold",bg="white", fg="red")
report2.place(x=1170,y=655)
##########################################################
